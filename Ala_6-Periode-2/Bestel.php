...
<?php foreach ($boeken as $boek) { ?>
    <div class="recipe-container">
        <div>
            ...
            <?php endif; ?>
            <?php
            $formattedPrice = number_format($boek['Boek_prijs'], 2, ',', '.');
            if ($boek['Status'] == 'korting') {
                echo "<span class='old-price strikethrough'>Oude prijs: €" . $formattedPrice . '</span><br>';
                echo "<span class='discount'>Korting: €" . number_format($boek['Korting_prijs'], 2, ',', '.') . '</span><br>';
                echo "<button class='btn' onclick='redirectToLink(" . $boek['Boek_id'] . ", \"" . $boek['Boek_link'] . "\")'>Bestellen</button>";
            } else {
                echo "<span class='price'>€" . $formattedPrice . '</span><br>';
                echo "<button class='btn' onclick='redirectToLink(" . $boek['Boek_id'] . ", \"" . $boek['Boek_link'] . "\")'>Bestellen</button>";
            }
            ?>
        </div>
        <?php if ($boek['Status'] == 'korting') : ?>
            <span class="ribbon">Korting</span>
        <?php endif; ?>
    </div>
<?php } ?>
 
<script>
function redirectToLink(boekId, boekLink) {
    console.log('Bestellen voor boekId: ' + boekId);
 
    // Voeg hier eventuele extra logica toe voordat je naar de link gaat
 
    window.location.href = boekLink;
}
</script>
